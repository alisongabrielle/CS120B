 ...
 *	Lab Section: 023
 *	Assignment: Lab # 11  Exercise # Advance 1
 *	Exercise Description: 
 *      player.h header file is also included. 
 *      The file consist of the menu for both single player and multi-player. 
 *      The menu's period was updated to ensure that the use is in the intro/menu state.
 *      The ball period when nearing both player paddle is also updated to be faster (near paddle) 
 *      or slower (near center).
 *      Multi-player and single player are both shown. 
 *      Keypad have some delays so I needed to restart the circuit but it should theoretically work. 
 *      Most button have some debouncing.
 *      Red LEDs are for player 2 or AI_player and White LEDs are for player 1. I made a slightly mistake 
 *      in the demo in the earlier part but I did mentioned in the video after about the leds. 
 *      
 *	I acknowledge all content contained herein, excluding template or example
 *	code, is my own original work.
 *
 *  Demo Link: https://youtu.be/KRr0-b89Up0
 */
...
unsigned char col_ball[] = {0x1E, 0x1D, 0x1B, 0x17, 0x0F, 0x00, 0x0F}; //Y AXIS
...
unsigned char fixer = 0;
unsigned char score = 0; //Red LED: player1 hit and wins , White LED: player2 or AI_player wins /hit

//-----------------end_of_variables-------------------------------  

enum Period_States {Period_Update};

int Period_SM (int state) {

    switch (state) { 
        case (Period_Update): break;
        default: state = Period_Update; break;
    }

    switch (state) {

        case (Period_Update):
            if (row_ball[ball.bPosRow] == 0x04 || row_ball[ball.bPosRow] == 0x02 || row_ball[ball.bPosRow] == 0x40 || row_ball[ball.bPosRow] == 0x20 ) {	                	
		ballPeriod = 300;
            }
            else {
                ballPeriod = 500;
    ...
    switch (state) {
        ...
        case (BallHit1_Check):
            if ( row_ball[ball.bPosRow] == 0x40 ) {           
                if ( moveRow[player1.currPos] == 0x1C && player1.ballHit == 0 ) {
                    if ( (col_ball[ball.bPosCol] == 0x1E ) || (col_ball[ball.bPosCol] == 0x1D) || (col_ball[ball.bPosCol] == 0x1B) ) {
                ...
                else if ( moveRow[player1.currPos] == 0x19 && player1.ballHit == 0 ) {
                    if ( (col_ball[ball.bPosCol] == 0x1E ) || (col_ball[ball.bPosCol] == 0x1D ) || (col_ball[ball.bPosCol] == 0x1B) || (col_ball[ball.bPosCol] == 0x17) ) {
                        player1.ballHit = 1;
                    }
                    else {
                ...
                else if ( moveRow[player1.currPos] == 0x13 && player1.ballHit == 0 ) {
                    if ( (col_ball[ball.bPosCol] == 0x1B ) || (col_ball[ball.bPosCol] == 0x17) || (col_ball[ball.bPosCol] == 0x0F) || (col_ball[ball.bPosCol] == 0x1D ) ) {
                        player1.ballHit = 1;
                    }
                    else {
                ...
                else if ( moveRow[player1.currPos] == 0x07 && player1.ballHit == 0 ) {
                    if ( (col_ball[ball.bPosCol] == 0x17 ) || (col_ball[ball.bPosCol] == 0x0F) || (col_ball[ball.bPosCol] == 0x1B) ) {
                        player1.ballHit = 1;   
                    }
                    else {
...
int Ball_Tick_Y_Axis(int state) {
	...
	switch (state) {
        ...
        case Ball_Y_shift:
            ...
            else if ( (AI_player.ballHit == 2 ) || (player1.ballHit == 2 ) || ( player2.ballHit == 2 ) ) {
                //debug = 0x01;
                state = Ball_Y_NotHit;
...
enum Ball_X_States {Ball_X_Wait, Ball_X_shift, Ball_X_HitAI, Ball_X_shiftBack, Ball_X_HitPl1, Ball_X_NotHit, Ball_X_rePlay};
int Ball_Tick_X_Axis(int state) {

    unsigned char start_again = ~PINB  & 0x80;
	...
	switch (state) {
	    ...
	    else if ( AI_player.ballHit == 2 || player2.ballHit == 2 || fixer == 1) { //AI_player miss ball
                ////debug = 0x02;
		fixer = 0;
                state = Ball_X_NotHit;
            }
            else if (AI_player.ballHit == 1 || player2.ballHit == 1) {
                ////debug = 0x01;
                score = 0x03;
...
int Player2_SM (int state) {
   ...
   switch (state) {
    ...
    switch (state){
        ...
        case (Player2_SMStart): player2.currPos = 1;

        case (Player2_Begin): player2.currPos = 1; break;

        case (Player2_Init): break;
        
        case (Player2_Inc):
         if (player2.currPos < numMove - 1) { 
            player2.currPos = player2.currPos + 1; }
            break;

        case (Player2_Dec):
         if (player2.currPos > 0) { 
            player2.currPos = player2.currPos - 1; }
            break;
...
enum AIplay_States {AIPlay_Init, AIplay_Wait, AIplay_Up, AIplay_Down};

int AIPlayer_Move(int state) {

    switch (state) {
        case (AIPlay_Init):
            state = ( player_mode == 1 ) ? AIplay_Wait: AIPlay_Init; break;
...
int TickFct_IncDec(int state) {
   ...
   switch (state) {
        case (IncDec_SMStart): 
            state = (start == 1 ) ? IncDec_Begin : IncDec_SMStart; break;
...
enum Display_States { Display_Wait, Display_Player1, Display_Player2, Display_Reset};
int Display_Tick(int state) {
	
   switch(state) {
      case Display_Wait: 
        if ( player_mode == 1) {
            state = Display_Player1;
        } 
        else if ( player_mode == 2 ){
            state = Display_Player2;
        }
        else {
            state = Display_Wait;
        }
    ...
    case Display_Player1:
        state = (player_mode == 0) ? Display_Reset : Display_Player1;
        break;

    case Display_Player2:
        state = (player_mode == 0) ? Display_Reset : Display_Player2;
        break;
   ...
   switch (state) {
        ...
        case Display_Player2:
            column[0] = moveRow[player1.currPos];
            column[1] = col_ball[ball.bPosCol];
            column[2] = moveRow[player2.currPos];
            row[0] = 0x80;
            row[1] = row_ball[ball.bPosRow];
            row[2] = 0x01;
        break;
...
int main(void) {
   ...
   task4.TickFct = &TickFct_IncDec;

   task5.state = start;
   task5.period = 200; 
   task5.elapsedTime = task5.period;
   task5.TickFct = &AIPlayer_Move;

   task6.state = start;
   task6.period = 100; 
   task6.elapsedTime = task6.period;
   task6.TickFct = &Player2_SM;

   task7.state = start;
   task7.period = 300; 
   task7.elapsedTime = task7.period;
   task7.TickFct = &Ball_Tick_X_Axis;

   task8.state = start;
   task8.period = 300; 
   task8.elapsedTime = task8.period;
   task8.TickFct = &Ball_Tick_Y_Axis;

   task9.state = start;
   task9.period = 300; 
   task9.elapsedTime = task9.period;
   task9.TickFct = &BallHit1_SM;

   task10.state = start;
   task10.period = 300; 
   task10.elapsedTime = task10.period;
   task10.TickFct = &BallHit2_SM;

   task11.state = start;
   task11.period = 300; 
   task11.elapsedTime = task11.period;
   task11.TickFct = &BallHitPlayer2_SM;

   task12.state = start;
   task12.period = 300; 
   task12.elapsedTime = task12.period;
   task12.TickFct = &Period_SM;
   ...
   while (1) {
      for ( i = 0; i < numTasks; i++ ) {
        ...
        tasks[2] -> period = startPeriod;
        tasks[6] -> period = ballPeriod;
        tasks[7] -> period = ballPeriod; 
